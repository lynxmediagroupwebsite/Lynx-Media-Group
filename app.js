...
document.querySelectorAll(".apply-form").forEach((form) => {
  form.addEventListener("submit", async (event) => {
    ...
    if (!applicationWebhookUrl) {
      showFormMessage(
        form,
        "Almost connected: the Sheet ID is saved, but this form still needs the deployed Google Apps Script web app URL in app.js.",
        "warning"
      );
      return;
    }
...
if ("IntersectionObserver" in window) {
  const demoObserver = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          startDemoFrame(entry.target);
        } else {
          pauseDemoFrame(entry.target);
        }
      });
    },
    { threshold: 0.55 }
